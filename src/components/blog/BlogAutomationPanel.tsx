...
export default function BlogAutomationPanel({ posts, onChanged }: { posts: BlogPost[]; onChanged: () => void }) {
  ...
  const [provider, setProvider] = useState({ provider: 'NVIDIA NIM', enabled: false, configured: false, model: 'qwen/qwen3.5-122b-a10b', baseUrlHost: 'integrate.api.nvidia.com' });
  ...
  const load = async () => {
    ...
    try {
      ...
      setProvider(dashboard.provider);
  ...
  return (
    <Panel className="p-5 sm:p-6">
      ...
      <div className="mt-6 rounded-lg border border-border p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><h4 className="font-semibold text-foreground">NVIDIA NIM provider</h4><p className="mt-1 text-xs text-muted-foreground">{provider.model} · {provider.baseUrlHost}. The API key is worker-only and never displayed.</p></div><div className="flex flex-wrap items-center gap-2"><StatusBadge tone={provider.configured ? 'success' : provider.enabled ? 'warning' : 'neutral'}>{provider.configured ? 'Configured' : provider.enabled ? 'Key required' : 'Disabled'}</StatusBadge><button type="button" disabled={Boolean(busy)} onClick={() => void run(() => testAdminBlogProvider(), 'Provider connectivity checked.', 'provider-test')} className="quiet-button">Test provider</button></div></div><p className="mt-3 text-xs text-muted-foreground">Review rollout: {overview.automaticApproved} approved of {settings.required_reviewed_articles_before_autopublish || 30} required. Strict Autopilot is {overview.strictAutopilotUnlocked ? 'available' : 'locked'}.</p></div>
      <label className="mt-3 flex items-start gap-3 text-sm"><input type="checkbox" checked={Boolean(settings.provider_enabled)} onChange={(event) => setSettings((value) => ({ ...value, provider_enabled: event.target.checked }))} className="mt-1 h-4 w-4" /><span><span className="font-semibold text-foreground">Enable NVIDIA jobs after worker configuration</span><span className="block text-xs text-muted-foreground">This admin intent does not replace the worker-side `NVIDIA_BLOG_ENABLED` safety switch.</span></span></label>
